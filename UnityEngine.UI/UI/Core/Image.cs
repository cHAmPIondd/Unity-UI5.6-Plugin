...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        /// <summary>
        /// 获取图像尺寸，shouldPreserveAspect是否保持宽高比例
        /// </summary>
        /// <param name="shouldPreserveAspect">是否保持宽高比例</param>
        /// <returns></returns>
        ...
        Vector4 GetAdjustedBorders(Vector4 border, Rect rect)
        {
            for (int axis = 0; axis <= 1; axis++)
            {
                ...
                //如果大小比边界还小，则按比例缩小边界
                float combinedBorders = border[axis] + border[axis + 2];//左+右|上+下
                if (rect.size[axis] < combinedBorders && combinedBorders != 0)//宽度<左+右|高度<上+下
                {
